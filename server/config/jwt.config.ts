...
function generatePayload<T>(user: T extends object? T: never): ToBeSignedPayload {
  if (isLike<{ role: any; _id: any }>(user, ["role", "_id"])) {
    return { role: user.role.toString(), id: user._id.toString()}
  }
  if (isLike<{ role: any; id: any }>(user, ["role", "id"])) {
...
function generateAccessToken<T>( user: T extends object? T: never): string {
  const payload: ToBeSignedPayload = generatePayload<T>(user);
...
function generateRefreshToken<T>(user: T extends object? T: never): string {
  const payload: ToBeSignedPayload = generatePayload<T>(user);
