...
async function refreshAccessToken(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    res
      ...
      .send({ isTokenResfreshed: true, accessToken: newAccessToken });
    return undefined;
  } catch (error) {
    return next(error);
  }
...
async function signInWithProductKey(req: Request, res: Response, next: NextFunction) {
  ...
  if (error) {
    return next(
      new HTTPError('ERROR_WHEN_CHECK_PRODUCT_KEY', error, HTTP_STATUS_CODE.UNAUTHORIZED, {})
    );
  }
...
export default {
  refreshToken: asyncHelper(refreshAccessToken),
